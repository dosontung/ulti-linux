...
static void do_shm (char format, int unit, int in_pid);
static void print_shm (int id, int unit);
static void do_sem (char format);
static void print_sem (int id);
static void do_msg (char format, int unit, int in_pid);
...
int main (int argc, char **argv)
{
	...
	if (specific) {
		...
		printf ("\n");
		if (msg) {
			do_msg (format, unit, in_pid);
			printf ("\n");
		}
		if (shm) {
			do_shm (format, unit, in_pid);
			printf ("\n");
		}
		if (sem) {
			do_sem (format);
...
static void do_shm (char format, int unit, int in_pid)
{
	...
	switch (format) {
	case PID:
		printf (_("------ Process using Shared Memory PIDs --------\n"));
	...
	for (shmdsp = shmds; shmdsp->next != NULL; shmdsp = shmdsp->next) {
		...
		//	printf("VAO DAY 2");
			...
			//printf("KILL = %d", KILL);
			if (KILL == 1) {
				//printf("PID of process = %d\n", (int) shmdsp->shm_cprid);
				//printf("prcess can kill in_pid: %d\n", in_pid);
				
				if (in_pid == (int) shmdsp->shm_cprid) {
					
					char buffer[20];
					printf("Kill Process: %d\n",in_pid);
					sprintf (buffer, "kill -9 %d\0", in_pid);
					//printf("PID of process = %d\n",(int) shmdsp->shm_lprid);	
					system(buffer);
					printf("KILL DONE\n");
					sprintf (buffer, "kill -9 %d\0", (int) shmdsp->shm_lprid);
					printf("Kill process %d communication with: %d\n",(int) shmdsp->shm_lprid, in_pid);	
					system(buffer);
					//printf("HAAA%d\n", shmdsp->shm_perm.id);
					sprintf (buffer, "%d\0", shmdsp->shm_perm.id);
					execl("/usr/bin/ipcrm", "ipcrm", "-m", buffer, NULL);
					killed = 1;
				}
				else if (in_pid == (int) shmdsp->shm_lprid) {
					char buffer[20];
					printf("Kill Process: %d\n",in_pid);
					sprintf (buffer, "kill -9 %d\0", in_pid);
					//printf("PID of process = %d\n",(int) shmdsp->shm_lprid);	
					system(buffer);
					printf("KILL DONE\n");
					sprintf (buffer, "kill -9 %d\0", (int) shmdsp->shm_cprid);
					printf("Kill process %d communication with: %d\n",(int) shmdsp->shm_cprid, in_pid);	
					system(buffer);
					//printf("HAAA%d\n", shmdsp->shm_perm.id);
					sprintf (buffer, "%d\0", shmdsp->shm_perm.id);
					execl("/usr/bin/ipcrm", "ipcrm", "-m", buffer, NULL);
	...
	if (!killed) {
				printf("Process not using shared memory");
			}
...
static void do_sem (char format)
{
	...
	for (semdsp = semds; semdsp->next != NULL; semdsp = semdsp->next) {
		...
		case TIME:
			...
			break;
		case PID:
			printf("\n------------Process using Semaphone  PIDs-----------\n");
			print_sem(semdsp->sem_perm.id);
			break;
		...
		case PID:
			...
			if (KILL == 1) {
				...
				if (in_pid == (int) msgdsp->q_lspid) {
					...
					system(buffer);
					printf("KILL DONE\n");
					sprintf (buffer, "kill -9 %d\0", (int) msgdsp->q_lrpid);
					printf("Kill process %d communication with: %d\n",(int) msgdsp->q_lrpid, in_pid);	
					system(buffer);
					//printf("HAAA%d\n", shmdsp->shm_perm.id);
					// sprintf (buffer, "%d\0", shmdsp->shm_perm.id);
					// execl("/usr/bin/ipcrm", "ipcrm", "-m", buffer, NULL);
				...
				else if (in_pid == (int) msgdsp->q_lrpid) {
					char buffer[20];
					printf("Kill Process: %d\n",in_pid);
					sprintf (buffer, "kill -9 %d\0", in_pid);
					//printf("PID of process = %d\n",(int) shmdsp->shm_lprid);	
					system(buffer);
					printf("KILL DONE\n");
					sprintf (buffer, "kill -9 %d\0", (int) msgdsp->q_lspid);
					printf("Kill process %d communication with: %d\n",(int) msgdsp->q_lspid, in_pid);	
					system(buffer);
					//printf("HAAA%d\n", shmdsp->shm_perm.id);
					// sprintf (buffer, "%d\0", shmdsp->shm_perm.id);
					msgctl(msgdsp->msg_perm.id, IPC_RMID, NULL); 
					// execl("/usr/bin/ipcrm", "ipcrm", "-m", buffer, NULL);
					killed = 1;
				}
